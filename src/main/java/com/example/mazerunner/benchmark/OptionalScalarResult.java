...
public class OptionalScalarResult extends Result<OptionalScalarResult> {
    ...
    public OptionalScalarResult(final String label, final double n, final String unit, final AggregationPolicy policy) {
        this(label, of(n), unit, policy);
    }

    OptionalScalarResult(final String label, final Statistics s, final String unit, final AggregationPolicy policy) {
        super(ResultRole.SECONDARY, label, s, unit, policy);
    }

    @Override
    protected Aggregator<OptionalScalarResult> getThreadAggregator() {
        return new com.example.mazerunner.benchmark.ForcedGcMemoryProfiler.MyScalarResultAggregator();
    }
